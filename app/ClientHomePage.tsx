...
export default function TraumLandingPage({ data, globalConfig }: { data: any; globalConfig?: any }) {
  ...
  const desktopBg = data?.hero?.desktopBackgroundImage || '/fondo.png'
  const mobileBg = data?.hero?.mobileBackgroundImage || '/fondo-completo-mobile.png'
  ...
  return (
    <main className="relative overflow-x-hidden bg-[#FAF6EE]">
      ...
      {/* Hero */}
      <section
        className="hero-bg-dynamic relative min-h-screen flex items-center justify-center overflow-hidden pb-24 bg-[position:30%_center] md:bg-[position:center]"
        style={{
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-2 md:pl-2 md:pr-12 flex flex-col items-center text-center md:items-start md:text-left md:translate-y-12">
          ...
          <div className="flex flex-col items-center md:items-start gap-y-2 w-full">
            ...
            <h1 className="overflow-hidden shrink-0 flex justify-center md:justify-start w-full">
              {(data?.hero?.title || "TRAUM").split("").map((char: string, i: number) => (
                <span
                  key={i}
            ...
            >
              {data?.hero?.subtitle ? (
                <div dangerouslySetInnerHTML={{ __html: data.hero.subtitle.replace(/diseño|cultura/gi, match => `<span class="font-semibold" style="color: ${colors.dark}">${match}</span>`).replace(/Donde el | encuentra la /gi, match => `<span style="color: ${colors.moss}">${match}</span>`) }} />
              ) : (
                <>
      ...
      >
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="mb-16 md:mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8 border-b pb-12" style={{ borderColor: `${colors.moss}30` }}>
            <div>
              ...
              >
                ...
                <span className="text-xs tracking-[0.3em] uppercase font-bold" style={{ color: colors.leaf }}>
                  {data?.about?.eyebrow || "Esencia Traum"}
                </span>
              </div>
              <h2
              ...
              >
                {data?.about?.title || "Más que una marca, un manifiesto visual."}
              </h2>
            </div>
            ...
            >
              {data?.about?.description || "Cada pieza guarda un fragmento del viaje, una historia esperando ser contada por quien la porta."}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 md:gap-24">
            ...
            >
              <div className="flex items-center gap-6 mb-8">
                ...
                <h3 className="text-2xl md:text-3xl font-bold tracking-tight" style={{ color: colors.dark }}>
                  {data?.about?.concept?.title || "El Concepto"}
                </h3>
              </div>
              <div className="prose prose-lg" style={{ color: `${colors.dark}AA` }}>
                <p className="text-lg leading-relaxed">
                  {data?.about?.concept?.content || "Nacemos en la intersección donde el diseño contemporáneo, la riqueza cultural y la narrativa personal convergen para crear algo único. No hacemos simplemente ropa o accesorios; diseñamos objetos con propósito y significado."}
                </p>
              </div>
            </div>
            ...
            >
              <div className="flex items-center gap-6 mb-8">
                ...
                <h3 className="text-2xl md:text-3xl font-bold tracking-tight" style={{ color: colors.dark }}>
                  {data?.about?.origin?.title || "El Origen"}
                </h3>
              </div>
              <div className="prose prose-lg" style={{ color: `${colors.dark}AA` }}>
                <p className="text-lg leading-relaxed">
                  {data?.about?.origin?.content ? (
                    <span dangerouslySetInnerHTML={{ __html: data.about.origin.content.replace(/laboratorio de ideas/i, match => `<span class="font-semibold" style="color: ${colors.leaf}">${match}</span>`) }} />
                  ) : (
                    <>
          ...
          >
            <div>
              <span className="text-xs font-bold uppercase tracking-[0.3em] block mb-4" style={{ color: colors.cream }}>
                {data?.vision?.eyebrow || "El Futuro"}
              </span>
              <p className="text-2xl md:text-3xl font-light" style={{ color: '#FAF6EE' }}>
                {data?.vision?.text || '"Algo grande se está gestando en nuestro taller."'}
              </p>
            </div>
